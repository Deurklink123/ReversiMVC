        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)   
                {
                    ...
                    speler.Guid = user.Id;
                    speler.Naam = user.UserName;
                    speler.AantalGewonnen = 0;
                    speler.AantalGelijk = 0;
                    speler.AantalVerloren = 0;
                    await _SpelerContext.AddAsync(speler);
                    await _SpelerContext.SaveChangesAsync();

                    var claim = new Claim(ClaimTypes.Role, "Speler");
                    var thisUser = await _userManager.FindByEmailAsync(Input.Email);
                    await _userManager.AddClaimAsync(thisUser, claim);
